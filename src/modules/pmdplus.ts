...
export class PmdPlus {
     ...
     */
    public async runPMD(targetFile: string, collection: vscode.DiagnosticCollection, progress?: vscode.Progress<{ message?: string; increment?: number }>, token?: vscode.CancellationToken ): Promise<void> {
    ...
    private async pmdPathIsValid(): Promise<boolean> {
        if(await Utilities.dirExists(this.configuration.pathToPmdExecutable)){
            ...
            vscode.window.showErrorMessage(`PMD+ could not find or access the PMD executable at ${this.configuration.pathToPmdExecutable}`);
        }
        return false;
    }
